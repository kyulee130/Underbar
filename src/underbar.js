(function() {
  ...
  _.last = function(array, n) {
    var indexFirst = n > array.length ? 0 : array.length - n;
    return n === undefined ? array[array.length - 1] : array.slice(indexFirst, array.length);
  };
  ...
  _.reduce = function(collection, iterator, accumulator) {
    if (Array.isArray(collection)) {
      //arguments is referring to the variables within the function.
      //If the collection is an array
      //If the accumulator argument exists in the beginning
      if (arguments.length === 3) {
        for (var i = 0; i < collection.length; i++) {
          accumulator = iterator(accumulator, collection[i]);
        }
      } //If the accumulator argument doesn't exist in the beginning
      else {
      ...
      return accumulator;
    } //If the collection is not an array and an object & the accumulator exists in the beginning
      else {
        if (arguments.length === 3) {
          for (var key in collection) {
            accumulator = iterator(accumulator, collection[key]);
          }
        } //If the collection is an array and an objet & the accumulator does not exist in the beginning
        else {
          var first = true;
          for (var key in collection) {
            if (first === true) {
              accumulator = collection[key];
              //first has to turn into false because the iteration will continue and the second number of the collection won't be first;
              first = false;
            } else {
              accumulator = iterator(accumulator, collection[key]);
            }
